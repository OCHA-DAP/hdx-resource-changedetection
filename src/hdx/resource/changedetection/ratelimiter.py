...
class RateLimiter:
    ...
    async def wait_for_token(self, host: str) -> None:
        ...
        if host not in self.host_limiters:
            self.host_limiters[host] = [self.MAX_TOKENS, self.start_time]
        while self.host_limiters[host][0] < 1:
            self.add_new_tokens(host)
            await asyncio.sleep(0.1)
        self.host_limiters[host][0] -= 1
    ...
    def add_new_tokens(self, host: str) -> None:
        ...
        async with self._lock:
            now = time.monotonic()
            time_since_update = now - self.host_limiters[host][1]
            new_tokens = time_since_update * self.RATE
            total_tokens = self.host_limiters[host][0] + new_tokens
            if total_tokens >= 1:
                self.host_limiters[host][0] = min(
                    total_tokens, self.MAX_TOKENS
                )
                self.host_limiters[host][1] = now
